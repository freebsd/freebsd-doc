...
[[help-statistics]]
== Statistiques de déploiement de FreeBSD

Bien que FreeBSD ne récupère pas d'informations statistiques de déploiement, disposer d'informations statistiques est essentiel.  Veuillez envisager d'installer le paquet https://www.freshports.org/sysutils/bsdstats/[sysutils/bsdstats], qui collecte des statistiques sur le matériel et les logiciels, afin d'aider les développeurs à orienter leurs efforts.  L'information collectée est disponible sur le site http://bsdstats.org[bsdstats.org].
