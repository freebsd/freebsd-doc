---
sidenav: download
title: Информация об аппаратной совместимости FreeBSD 14.4
---
...
Эта заметка об аппаратной совместимости описывает всё оборудование, которое, как известно, работает с FreeBSD {localRel}. Другие конфигурации также могут работать, но просто ещё не были протестированы. Приветствуются отзывы, обновления и исправления этого списка!
...
Поддержка RISC-V представляет собой важную веху в эволюции этой операционной системы с открытым исходным кодом. FreeBSD была первой операционной системой, получившей загружаемую поддержку RISC-V в дереве исходного кода, что подчёркивает её приверженность поддержке новых аппаратных архитектур.
...
В этом разделе описываются устройства, которые, как известно в настоящее время, поддерживаются каждым драйвером устройств в FreeBSD. Если рассматриваемый драйвер имеет страницу руководства в базовом дистрибутиве FreeBSD (большинство должно иметь), она указана здесь. Информация об использовании, конфигурации и конкретных моделях поддерживаемых устройств, контроллеров и т.д. может быть найдена на страницах руководств.

[NOTE]
====
Списки устройств в этом документе генерируются автоматически из страниц руководств FreeBSD. Это означает, что некоторые устройства, которые поддерживаются несколькими драйверами, могут появляться несколько раз.
====
...
Драйвер **ahci** поддерживает AHCI-совместимые контроллеры, имеющие PCI класс 1 (массовое хранилище), подкласс 6 (SATA) и программный интерфейс 1 (AHCI).
...
Драйвер **arcmsr** поддерживает следующие карты:
...
Драйвер **ata** поддерживает интерфейс IDE на следующих ATA/SATA-контроллерах:
...
Драйвер **ciss** поддерживает контроллеры, реализующие Common Interface for SCSI-3 Support Open Specification v1.04, включая:
...
Драйвер **hpt27xx** поддерживает следующие SAS контроллеры:
...
Драйвер **hptiop** поддерживает следующие SAS и SATA RAID-контроллеры:
...
Драйвер **hptiop** также поддерживает следующие SAS и SATA RAID-контроллеры, которые уже достигли конца срока службы:
...
Драйвер **hptmv** поддерживает следующие ATA RAID контроллеры:

*	HighPoint's RocketRAID 182x series



Драйвер **hptnr** поддерживает следующие SATA контроллеры:
...
Драйвер **hptrr** поддерживает следующие RAID контроллеры:
...
Следующие контроллеры поддерживаются драйвером **ida**:
...
Контроллеры, поддерживаемые драйвером **ips** , включают:
...
Драйвер **mfi** поддерживает следующее оборудование:
...
Драйвер **mlx** поддерживает следующие Parallel SCSI RAID-контроллеры:
...
Эти контроллеры поддерживаются драйвером **mpi3mr**:
...
Эти контроллеры поддерживаются драйвером **mps**:
...
Следующие контроллеры поддерживаются драйвером **mpt**:
...
Драйвер **mrsas** поддерживает следующие LSI/Broadcom SATA/SAS RAID-контроллеры:
...
Драйвер **mvs** поддерживает следующие контроллеры:
...
Драйвер **ocs_fc** поддерживает эти адаптеры Fibre Channel:
...
Драйвер **pms** поддерживает следующее оборудование:
...
Драйвер **pst** поддерживает контроллер аппаратного RAID Promise Supertrak SX6000 ATA.
...
Драйвер **siis** поддерживает следующие чипы контроллеров:
...
Контроллеры, поддерживаемые драйвером **smartpqi**, включают, но не ограничиваются:
...
Драйвер **sym** обеспечивает поддержку следующих Symbios/LSI Logic PCI SCSI контроллеров:
...
Драйвер **tws** поддерживает следующий SATA/SAS RAID-контроллер:
...
Драйвер **ae** поддерживает контроллеры Attansic/Atheros L2 PCIe FastEthernet и, как известно, работает со следующим оборудованием:
...
Драйвер **age** обеспечивает поддержку встроенных сетевых контроллеров (LOM) на основе чипов контроллера Ethernet Attansic/Atheros L1 Gigabit, включая:
...
Драйвер устройства **alc** обеспечивает поддержку следующих контроллеров Ethernet:
...
Драйвер устройства **ale** обеспечивает поддержку следующих контроллеров Ethernet:
...
Драйвер **aue** поддерживает следующие USB-адаптеры Fast Ethernet на основе чипсета ADMtek AN986 Pegasus:
...
Драйвер **axe** поддерживает USB-адаптеры Ethernet на основе ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760, включая:
...
Драйвер **axge** поддерживает следующие USB-контроллеры Gigabit Ethernet:
...
Драйвер **bce** обеспечивает поддержку различных сетевых карт на основе семейства контроллеров Gigabit Ethernet QLogic NetXtreme II, включая следующие:
...
Драйвер **bge** обеспечивает поддержку различных сетевых карт на основе семейства чипов контроллера Gigabit Ethernet Broadcom BCM570x, включая следующие:
...
Драйвер **bnxt** обеспечивает поддержку различных сетевых карт на основе семейств контроллеров Gigabit Ethernet Broadcom NetXtreme-C и NetXtreme-E, включая следующие:
...
Драйвер **bxe** обеспечивает поддержку различных сетевых карт на основе семейства чипов контроллера 10Gb Ethernet QLogic NetXtreme II, включая следующие:
...
Чипы, поддерживаемые драйвером **cas**:

*	National Semiconductor DP83065 Saturn Gigabit Ethernet
*	Sun Cassini Gigabit Ethernet
*	Sun Cassini+ Gigabit Ethernet

Следующие дополнительные карты, как известно, на данный момент работают с драйвером **cas**:
...
Драйвер **cdce** поддерживает USB-интерфейсы Ethernet, реализующие протокол USB Communication Device Class Ethernet Control Model (CDC ECM) или Network Control Model (CDC NCM), такие как:
...
Драйвер **cue** поддерживает USB-адаптеры Ethernet на основе CATC USB-EL1210A, включая:
...
Драйвер **cxgb** поддерживает адаптеры Ethernet 10 Gigabit и 1 Gigabit на основе чипсетов T3 и T3B:
...
Драйвер **cxgbe** поддерживает адаптеры Ethernet 100Gb и 25Gb на основе ASIC T6:
...
Драйвер **cxgbe** поддерживает адаптеры Ethernet 40Gb, 10Gb и 1Gb на основе ASIC T5:
...
Драйвер **cxgbe** поддерживает адаптеры Ethernet 10Gb и 1Gb на основе ASIC T4:
...
Драйвер **cxgbev** поддерживает Виртуальные Функции на адаптерах Ethernet 100Gb и 25Gb на основе ASIC T6:
...
Драйвер **cxgbev** поддерживает Виртуальные Функции на адаптерах Ethernet 40Gb, 10Gb и 1Gb на основе ASIC T5:
...
Драйвер **cxgbev** поддерживает Виртуальные Функции на адаптерах Ethernet 10Gb и 1Gb на основе ASIC T4:
...
Драйвер **dc** обеспечивает поддержку следующих чипсетов:
...
Следующие сетевые карты, как известно, на данный момент работают с драйвером **dc**:
...
Встроенные контроллеры Gigabit Ethernet в следующих системах-на-кристалле Freescale, как известно, работают с драйвером **dtsec**:
...
Драйвер **enic** должен поддерживать все известные карты Cisco VIC.




Драйвер **et** поддерживает адаптеры Ethernet Agere ET1310 10/100/Gigabit.





Адаптеры, поддерживаемые драйвером **fxp** , включают:
...
Чипы, поддерживаемые драйвером **gem** , включают:

*	Apple GMAC
*	Sun GEM Gigabit Ethernet

Следующие дополнительные карты, как известно, на данный момент работают с драйвером **gem**:
...
Драйвер **genet** поддерживает часть контроллера Ethernet в Broadcom BCM2711 на Raspberry Pi 4 Model B и связанных системах. Он использует PHY BCM54213PE.
...
The **ice** driver supports the Intel Ethernet 800 series. Most adapters in this series with SFP28/QSFP28 cages have firmware that requires that Intel qualified modules are used; these qualified modules are listed below. This qualification check cannot be disabled by the driver.

Драйвер **ice** поддерживает адаптеры Ethernet 100Gb с этими модулями QSFP28:



Драйвер **igc** поддерживает следующие модели:
...
Следующие устройства поддерживаются драйвером **ipheth**:
...
Драйвер **ix** поддерживает адаптеры Intel 10Gb Ethernet PCIe, включая:
...
Драйвер **ixl** поддерживает серию Intel Ethernet 700. Большинство адаптеров этой серии с модулями SFP+/SFP28/QSFP+ имеют прошивку, требующую использования квалифицированных модулей Intel; эти квалифицированные модули перечислены ниже. Эта проверка квалификации не может быть отключена драйвером.

Драйвер **ixl** поддерживает адаптеры Ethernet 40Gb с этими модулями QSFP+:

*	Intel 4x10G/40G QSFP+ 40GBASE-SR4 E40GQSFPSR
*	Intel 4x10G/40G QSFP+ 40GBASE-LR4 E40GQSFPLR

Драйвер **ixl** поддерживает адаптеры Ethernet 25Gb с этими модулями SFP28:

*	Intel 10G/25G SFP28 25GBASE-SR E25GSFP28SR
*	Intel 10G/25G SFP28 25GBASE-SR E25GSFP28SRX (Extended Temp)

Драйвер **ixl** поддерживает адаптеры Ethernet 25Gb и 10Gb с этими модулями SFP+:
...
Драйвер устройства **jme** обеспечивает поддержку следующих контроллеров Ethernet:
...
Драйвер **kue** поддерживает USB-адаптеры Ethernet на основе Kawasaki LSI KL5KLUSB101B, включая:
...
Адаптеры, поддерживаемые драйвером **lge** , включают:
...
Драйвер **liquidio** поддерживает следующие карты:
...
Драйвер **mlx4en** поддерживает следующие сетевые адаптеры:
...
Драйвер **mlx4ib** поддерживает следующие сетевые адаптеры:
...
Драйвер **mlx5en** поддерживает адаптеры Ethernet 200Gb, 100Gb, 50Gb, 40Gb, 25Gb и 10Gb.
...
Драйвер **mlx5ib** поддерживает сетевые адаптеры 100Gb, 50Gb, 40Gb, 25Gb и 10Gb. ConnectX-4 поддерживает: скорости 10/20/25/40/50/56/100Gb/с. ConnectX-4 LX поддерживает: скорости 10/25/40/50Gb/с (и сниженное энергопотребление):
...
Адаптеры, поддерживаемые драйвером **mos** , включают:

*	Sitecom LN030



Драйвер **msk** обеспечивает поддержку различных сетевых карт на основе чипов контроллера Gigabit Ethernet Marvell/SysKonnect Yukon II, включая:
...
Драйвер **mxge** поддерживает адаптеры 10 Gigabit Ethernet на основе чипов Myricom LANai Z8E:
...
Драйвер **my** обеспечивает поддержку различных сетевых карт на основе чипсета Myson. Поддерживаемые модели включают:
...
Драйвер **nfe** поддерживает следующие встроенные адаптеры NVIDIA MCP:
...
Драйвер **nge** поддерживает адаптеры Gigabit Ethernet на основе National Semiconductor DP83820 и DP83821, включая:
...
Драйвер **oce** поддерживает следующие сетевые адаптеры:
...
Драйвер **qlnxe** поддерживает адаптеры 25/40/100 Gigabit Ethernet & CNA на основе следующих чипсетов:
...
Драйвер **qlxgb** поддерживает адаптеры 10 Gigabit Ethernet & CNA на основе следующих чипсетов:
...
Драйвер **qlxgbe** поддерживает адаптеры 10 Gigabit Ethernet & CNA на основе следующих чипсетов:

*	QLogic 8300 series



Драйвер **qlxge** поддерживает адаптеры 10 Gigabit Ethernet & CNA на основе следующих чипсетов:

*	QLogic 8100 series



Драйвер **re** поддерживает адаптеры Fast Ethernet и Gigabit Ethernet на основе RealTek RTL8139C+, RTL8169, RTL816xS, RTL811xS, RTL8168, RTL810xE и RTL8111, включая:
...
Адаптеры, поддерживаемые драйвером **rl** , включают:
...
Драйвер **rue** поддерживает USB-адаптеры Ethernet на основе RealTek RTL8150, включая:
...
Драйвер **sfxge** поддерживает все адаптеры 10Gb Ethernet на основе контроллеров семейства Solarflare SFC9000.



Драйвер устройства **sge** обеспечивает поддержку следующих контроллеров Ethernet:
...
Драйвер **sis** поддерживает адаптеры Fast Ethernet и встроенные контроллеры на основе Silicon Integrated Systems SiS 900 и SiS 7016, а также адаптеры Fast Ethernet на основе чипов National Semiconductor DP83815 (MacPhyter) и DP83816. Поддерживаемые адаптеры включают:
...
Адаптеры, поддерживаемые драйвером **sk** , включают:
...
Следующие устройства поддерживаются драйвером **smsc**:
...
Драйвер **ste** поддерживает адаптеры Fast Ethernet и встроенные контроллеры на основе Sundance Technologies ST201, включая:
...
Драйвер **stge** обеспечивает поддержку различных сетевых карт на основе чипов контроллера Gigabit Ethernet Sundance/Tamarack TC9021, включая:
...
Драйвер **ti** поддерживает адаптеры Gigabit Ethernet на основе чипов Alteon Tigon I и II. Драйвер **ti** тестировался со следующими адаптерами:
...
Следующие адаптеры также должны поддерживаться, но еще не тестировались:
...
Встроенные контроллеры Gigabit Ethernet в следующих системах-на-кристалле Freescale, как известно, работают с драйвером **tsec**:
...
Драйвер **udav** поддерживает следующие адаптеры:
...
Устройства, поддерживаемые драйвером **ufoma** , включают:
...
Драйвер **uhso** должен работать с большинством устройств от Option. Следующие устройства проверены на работоспособность:
...
Драйвер **ure** поддерживает следующие USB-контроллеры Ethernet:
...
Драйвер **urndis** поддерживает функциональность "модема" (tethering) многих устройств Android.



Драйвер **vge** поддерживает адаптеры Gigabit Ethernet на основе VIA Networking VT6120, VT6122, VT6130 и VT6132, включая:
...
Драйвер **vr** поддерживает адаптеры Fast Ethernet на основе VIA Technologies Rhine I, Rhine II и Rhine III, включая:
...
Драйвер устройства **vte** обеспечивает поддержку следующих контроллеров Ethernet:
...
Драйвер **xl** поддерживает следующее оборудование:
...
Драйвер **ar40xx_switch** поддерживает следующие контроллеры коммутаторов Gigabit Ethernet:
...
Драйвер **arswitch** поддерживает следующие контроллеры коммутаторов Ethernet:
...
Драйвер **e6000sw** поддерживает следующие контроллеры коммутаторов Gigabit Ethernet:
...
Драйвер **e6060sw** поддерживает следующие контроллеры коммутаторов Fast Ethernet:
...
Драйвер **ip17x** поддерживает следующие контроллеры коммутаторов Fast Ethernet:
...
Драйвер **mtkswitch** поддерживает следующие контроллеры коммутаторов Ethernet:
...
Драйвер **ath** поддерживает все карты Atheros Cardbus, ExpressCard, PCI и PCIe, за исключением тех, которые основаны на чипсете AR5005VL.



Следующие карты входят в число поддерживаемых модулем **ath_hal**:
...
Драйвер **bwi** поддерживает беспроводные устройства на основе Broadcom BCM43xx, включая:
...
Драйвер **bwn** поддерживает беспроводные устройства на основе Broadcom BCM43xx, включая:
...
Драйвер **ipw** обеспечивает поддержку сетевого адаптера Intel PRO/Wireless 2100a/b MiniPCI.



Драйвер **iwi** поддерживает следующие беспроводные сетевые устройства:
...
Драйвер **iwlwifi** поддерживает устройства PCIe из под-драйвера **mvm** со следующими поколениями чипсетов:
...
Драйвер **iwlwifi** поддерживает устройства PCIe из под-драйвера **mld** со следующими поколениями чипсетов:
...
Драйвер **iwm** поддерживает следующие устройства Wi-Fi PCIe:
...
Драйвер **iwn** обеспечивает поддержку:
...
Следующие карты входят в число поддерживаемых драйвером **malo**:
...
Драйвер **otus** обеспечивает поддержку беспроводных сетевых адаптеров Atheros AR9170 USB IEEE 802.11b/g/n, включая:
...
Драйвер **ral** поддерживает беспроводные адаптеры PCI/PCIe/CardBus на основе чипсетов Ralink Technology, включая:
...
Драйвер **rsu** обеспечивает поддержку беспроводных сетевых адаптеров Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n, включая:
...
Драйвер **rtw88** поддерживает устройства PCIe со следующими чипсетами:
...
Драйвер **rtw89** поддерживает устройства PCIe со следующими чипсетами:
...
Драйвер **rtwn_pci** поддерживает следующие устройства Wi-Fi PCIe:
...
Драйвер **rtwn_usb** поддерживает USB-адаптеры беспроводной сети на основе некоторых чипсетов Realtek RTL 8188/8192/8812 и 8821, включая:
...
Драйвер **rum** поддерживает USB 2.0 беспроводные адаптеры на основе чипсетов Ralink RT2501USB и RT2601USB, включая:
...
Драйвер **run** поддерживает следующие беспроводные адаптеры:
...
Драйвер **uath** должен работать со следующими адаптерами:
...
Драйвер **upgt** поддерживает USB 2.0 беспроводные адаптеры серии Conexant/Intersil PrismGT на основе чипсета GW3887, среди них:
...
Драйвер **ural** поддерживает USB 2.0 беспроводные адаптеры на основе чипсета Ralink Technology RT2500USB, включая:
...
Драйвер **urtw** поддерживает беспроводные сетевые устройства на основе Realtek RTL8187B/L, включая:
...
Драйвер **wpi** обеспечивает поддержку сетевого адаптера Intel PRO/Wireless 3945ABG Mini PCIe.



Следующие устройства, как известно, поддерживаются драйвером **zyd**:
...
Драйвер **u3g** поддерживает следующие сотовые модемы:
...
Основной компонент и интерфейсы ядра взаимодействуют с оборудованием через аппаратный интерфейс. Этот интерфейс служит абстракцией оборудования и позволяет использовать различные UART для последовательной связи.


Драйвер **uart** поддерживает следующие классы UART:
...
Драйвер **scc** поддерживает следующие классы контроллеров последовательной связи:
...
The **puc** driver supports the following PCI multi-port serial and parallel adapters:
...
Драйвер **uark** поддерживает следующие адаптеры:
...
Драйвер **ubsa** поддерживает следующие адаптеры:
...
Драйвер **ubser** обеспечивает поддержку последовательных адаптеров управления консолью BWCT.



Драйвер **uchcom** поддерживает следующие USB-контроллеры UART для последовательного интерфейса:
...
Драйвер **ucycom** в настоящее время поддерживает следующие устройства, содержащие мостовые микросхемы Cypress USB to RS232:
...
Драйвер **uftdi** поддерживает следующие USB-контроллеры UART для последовательного интерфейса:
...
Драйвер **umcs** тестировался на следующих адаптерах:
...
Драйвер **umct** поддерживает следующие адаптеры:
...
Драйвер **uplcom** поддерживает следующие устройства и адаптеры:
...
Следующие устройства должны работать с драйвером **uslcom**:
...
Драйвер **uvscom** поддерживает следующие адаптеры:
...
Драйвер **snd_als4000** поддерживает следующие звуковые карты:

*	Avance Logic ALS4000



Драйвер **snd_atiixp** поддерживает следующие аудиочипсеты:
...
Драйвер **snd_cmi** поддерживает следующие звуковые карты:
...
Драйвер **snd_cs4281** поддерживает следующие звуковые карты:

*	Crystal Semiconductor CS4281



Драйвер **snd_csa** поддерживает следующие звуковые карты:
...
Некоторые встроенные чипы CS4610 сопровождаются кодеком CS423x ISA вместо кодека CS4297 AC97. Такие конфигурации пока не поддерживаются драйвером **snd_csa**.



Драйвер **snd_emu10k1** поддерживает следующие звуковые карты:
...
Драйвер **snd_emu10kx** поддерживает следующие звуковые карты:
...
Драйвер **snd_envy24** поддерживает следующие аудиоустройства:
...
Драйвер **snd_envy24ht** поддерживает следующие аудиоустройства:
...
Драйвер **snd_es137x** поддерживает следующие звуковые карты:
...
Драйвер **snd_fm801** поддерживает аудиоустройства на основе следующего чипсета:
...
Драйвер **snd_hdspe** поддерживает следующие аудиоустройства:
...
Драйвер **snd_ich** поддерживает следующие аудиоустройства:
...
Драйвер **snd_maestro3** поддерживает следующие аудиоустройства:
...
Драйвер **snd_neomagic** поддерживает следующие аудиоустройства:
...
Драйвер **snd_solo** поддерживает следующие звуковые карты:
...
Драйвер **snd_spicds** поддерживает следующие кодеки:
...
Драйвер **snd_t4dwave** поддерживает следующие аудиоустройства:
...
Драйвер **snd_via8233** поддерживает следующие аудиочипсеты:
...
Драйвер **snd_via82c686** поддерживает аудиоустройства на основе следующего чипсета:

*	VIA 82C686A



Драйвер **snd_vibes** поддерживает аудиоустройства на основе следующего чипсета:
...
Драйвер **atp** обеспечивает поддержку следующих Product ID:
...
Драйвер **atopcase** поддерживает следующие MacBooks, выпущенные в 2015-2018 годах:
...
Драйвер **abtn** поддерживает расширенные клавиши клавиатуры (специальные F-клавиши) на следующих устройствах:
...
Устройства, поддерживаемые драйвером **akbd** , включают:
...
Устройства, поддерживаемые драйвером **ams** , включают:
...
Драйвер **ohci** поддерживает все OHCI v1.0 совместимые контроллеры, включая:
...
Следующие устройства поддерживаются драйвером **ugold**:

*	RDing TEMPer1V1.2



Драйвер **uhci** поддерживает все UHCI v1.1 совместимые контроллеры, включая:
...
Драйвер **uipaq** поддерживает следующие устройства iPAQ:
...
Следующие устройства поддерживаются драйвером **urio**:
...
Устройства, поддерживаемые драйвером **umodem** , включают:
...
Драйвер **rtsx** поддерживает следующие кард-ридеры Realtek SD:
...
Драйвер **ulpt** обеспечивает поддержку USB-принтеров и кабелей преобразования параллельного принтера, включая следующие:
...
Драйвер **uvisor** поддерживает следующие устройства:
...
Драйвер **fwohci** обеспечивает поддержку карт интерфейса FireWire PCI/CardBus. Драйвер поддерживает следующие чипсеты IEEE 1394 OHCI:
...
Драйвер **ng_ubt** поддерживает все USB-устройства Bluetooth, соответствующие спецификации Bluetooth v1.1, включая:
...
Драйвер **ccr** поддерживает механизм криптографического ускорения, встроенный в адаптеры на основе ASIC T6:
...
Драйвер **hifn** поддерживает различные карты, содержащие чипсеты Hifn 7751, 7951, 7811, 7955 и 7956, такие как:
...
Драйвер **qat** поддерживает следующие механизмы Intel QuickAssist Technology:
...
Драйвер **safe** поддерживает следующие чипы SafeXcel:
...
Драйвер **safexcel** поддерживает функции криптографического ускорения устройства Inside Secure EIP-97, найденного в некоторых системах-на-кристалле Marvell.
...
Драйвер **ads111x** обеспечивает поддержку следующих устройств:
...
Драйвер **amdsbwd** поддерживает следующие чипсеты:
...
Драйвер **at45d** обеспечивает поддержку следующих устройств:
...
Драйвер **gpioths** обеспечивает поддержку следующих устройств:
...
Драйвер **intpm** поддерживает следующие чипсеты:
...
Текущая версия драйвера **netfpga10g_nf10bmac** работает с одним интерфейсом режима PIO ядра NetFPGA-10G Embedded CPU Ethernet версии 1.00a.



Следующие чипы PLX/Avago/Broadcom поддерживаются драйвером **ntb_hw_plx**:
...
Драйвер **superio** поддерживает множество контроллеров Super I/O, производимых Nuvoton (ранее известными как Winbond) и ITE, а именно:
...
//
// Original EN revision (08.02.2026): aa29975e70315b9c9b43ec5ee7d3aff7464331a2
//
