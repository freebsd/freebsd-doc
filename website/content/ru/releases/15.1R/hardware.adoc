...
The man:aac[4] driver supports the following Parallel SCSI, SATA, and 3G SAS RAID controllers from the Adaptec AAC family:
...
The man:aacraid[4] driver supports the following Adaptec 6G and 12G SAS/SATA RAID controllers:
...
The man:ahc[4] driver supports the following VL/ISA/PCI parallel SCSI controllers and cards:
...
The man:ahd[4] driver supports the following PCI/PCI-X parallel SCSI controllers:
...
The man:arcmsr[4] driver supports the following Areca PCI-X/PCIe SATA/SAS/NVMe RAID host adapters:
...
The man:ata[4] driver supports the IDE interface on the following ATA/SATA controllers:
...
The man:ciss[4] driver supports controllers implementing Common Interface for SCSI-3 Support Open Specification v1.04, including:
...
The man:hpt27xx[4] driver supports the following SAS controllers:
...
The man:hptiop[4] driver supports the following SAS and SATA RAID controllers:
...
The man:hptmv[4] driver supports the following ATA RAID controllers:

*	HighPoint's RocketRAID 182x series



The man:hptnr[4] driver supports the following SATA controllers:
...
The man:hptrr[4] driver supports the following RAID controllers:
...
The following controllers are supported by the man:ida[4] driver:
...
Controllers supported by the man:ips[4] driver include:
...
The man:isci[4] driver provides support for Intel C600 `SAS` controllers.



The man:isp[4] driver supports the following optical Fibre Channel adapters:
...
The man:mfi[4] driver supports the following hardware:
...
The man:mlx[4] driver supports the following Parallel SCSI RAID controllers:
...
The man:mmcsd[4] driver implements direct access block device for MMC and SD memory cards.



These controllers are supported by the man:mpi3mr[4] driver:
...
The man:mpr[4] driver supports the following SATA/SAS/NVMe RAID controllers:
...
These controllers are supported by the man:mps[4] driver:
...
The man:mrsas[4] driver supports the following LSI/Broadcom SATA/SAS RAID controllers:
...
The man:nda[4] driver supports NVMe (Non-Volatile Memory Express) storage devices connected via PCIe or over NVMF (NVMe over Fabric) via the CAM subsystem.



The man:nvd[4] driver supports NVMe storage devices using NVMe namespaces.



The man:ocs_fc[4] driver supports the following PCIe Fibre Channel adapters:
...
The man:pms[4] driver supports the following hardware:
...
The man:pst[4] driver supports the Promise Supertrak SX6000 ATA hardware RAID controller.



The man:sdhci[4] driver supports the SD Host Controller Specification. When attaching via the PCI bus, the controller is automatically configured. Many SoC chips provide a SDHCI controller directly mapped to I/O memory. For those, the controller may be configured using fdt(4) or acpi(4) methods, supplied by your board's vendor.
...
Controllers supported by the man:smartpqi[4] driver include, but not limited to:
...
The man:tws[4] driver supports the following SATA/SAS RAID controller:

*	LSI's 3ware SAS 9750 series



The man:ufshci[4] driver supports both host controllers and devices implementing the Universal Flash Storage Host Controller Interface 4.1 and earlier.
...
The man:alc[4] device driver provides support for the following Ethernet controllers:
...
The man:aue[4] driver supports the following USB Fast Ethernet adapters based on the ADMtek AN986 Pegasus chipset:
...
The man:axe[4] driver supports ASIX Electronics AX88172/AX88178/AX88772/AX88772A/AX88772B/AX88760 based USB Ethernet adapters including:
...
The man:axge[4] driver supports the following USB Gigabit Ethernet controllers:
...
The man:bce[4] driver provides support for various NICs based on the QLogic NetXtreme II family of Gigabit Ethernet controllers, including the following:
...
The man:bge[4] driver provides support for various NICs based on the Broadcom BCM570x family of Gigabit Ethernet controller chips, including the following:
...
The man:bnxt[4] driver supports the following Broadcom 10Gb to 400Gb Ethernet controllers:
...
The man:bxe[4] driver provides support for various NICs based on the QLogic NetXtreme II family of 10Gb Ethernet controller chips, including the following:
...
The man:cdce[4] driver supports USB Ethernet interfaces implementing the USB Communication Device Class Ethernet Control Model (CDC ECM) or Network Control Model (CDC NCM) protocol, such as:
...
The man:cue[4] driver supports CATC USB-EL1210A based USB Ethernet adapters including:
...
The man:cxgb[4] driver supports 10 Gigabit and 1 Gigabit Ethernet adapters based on the T3 and T3B chipset:
...
The man:cxgbe[4] driver supports 400Gb, 200Gb, 50Gb, and 10Gb Ethernet adapters based on the T7 ASIC:
...
The man:cxgbev[4] driver supports Virtual Functions on 100Gb and 25Gb Ethernet adapters based on the T6 ASIC:
...
Gigabit Ethernet controllers built into the following Freescale system-on-chip devices are known to work with the man:dtsec[4] driver:
...
The man:em[4] driver supports Gigabit Ethernet adapters based on the Intel 82540, 82541ER, 82541PI, 82542, 82543, 82544, 82545, 82546, 82546EB, 82546GB, 82547, 82571, 82572, 82573, 82574, 82575, 82576, and 82580 controller chips:
...
The man:enic[4] driver should supports all known Cisco VIC cards.




The man:et[4] driver supports Agere ET1310 10/100/Gigabit Ethernet adapters.
...
The man:genet[4] driver supports the Ethernet controller portion of the Broadcom BCM2711 on the Raspberry Pi 4 Model B and related systems. It utilizes the BCM54213PE PHY.
...
The man:ice[4] driver supports the following Intel 800 series 1Gb to 200Gb Ethernet controllers:
...
The man:igc[4] driver supports the following 2.5Gb Ethernet controllers:
...
The following devices are supported by the man:ipheth[4] driver:
...
The man:ix[4] driver supports Intel 10Gb Ethernet PCIe adapters, including:
...
The man:ixl[4] driver supports the Intel Ethernet 700 series. Most adapters in this series with SFP+/SFP28/QSFP+ cages have firmware that requires that Intel qualified modules are used; these qualified modules are listed below. This qualification check cannot be disabled by the driver.
...
The man:kue[4] driver supports Kawasaki LSI KL5KLUSB101B based USB Ethernet adapters including:
...
Adapters supported by the man:lge[4] driver include:
...
The man:liquidio[4] driver supports the following cards:
...
The man:mgb[4] driver supports Microchip PCIe Gigabit Ethernet interfaces, including:
...
The man:mlx4en[4] driver supports the following network adapters:
...
The man:mlx4ib[4] driver supports the following network adapters:
...
The man:mlx5en[4] driver supports 200Gb, 100Gb, 50Gb, 40Gb, 25Gb and 10Gb ethernet adapters.
...
The man:mlx5ib[4] driver supports 100Gb, 50Gb, 40Gb, 25Gb and 10Gb network adapters. ConnectX-4 supports: 10/20/25/40/50/56/100Gb/s speeds. ConnectX-4 LX supports: 10/25/40/50Gb/s speeds (and reduced power consumption):
...
Adapters supported by the man:mos[4] driver include:

*	Sitecom LN030



The man:msk[4] driver provides support for various NICs based on the Marvell/SysKonnect Yukon II based Gigabit Ethernet controller chips, including:
...
The man:muge[4] driver supports Microchip USB Gigabit Ethernet interfaces, including:
...
The man:mxge[4] driver supports 10 Gigabit Ethernet adapters based on the Myricom LANai Z8E chips:
...
The man:my[4] driver provides support for various NICs based on the Myson chipset. Supported models include:
...
The man:nfe[4] driver supports the following NVIDIA MCP onboard adapters:
...
The man:oce[4] driver supports the following network adapters:
...
The man:qlnxe[4] driver supports 25/40/100 Gigabit Ethernet & CNA Adapter based on the following chipsets:
...
The man:qlxgb[4] driver supports 10 Gigabit Ethernet & CNA Adapter based on the following chipsets:
...
The man:qlxgbe[4] driver supports 10 Gigabit Ethernet & CNA Adapter based on the following chipsets:

*	QLogic 8300 series



The man:qlxge[4] driver supports 10 Gigabit Ethernet & CNA Adapter based on the following chipsets:
...
The man:rue[4] driver supports RealTek RTL8150 based USB Ethernet adapters including:
...
The following devices are supported by the man:smsc[4] driver:
...
The man:ti[4] driver supports Gigabit Ethernet adapters based on the Alteon Tigon I and II chips. The **ti** driver has been tested with the following adapters:
...
Gigabit Ethernet controllers built into the following Freescale system-on-chip devices are known to work with the man:tsec[4] driver:
...
The man:udav[4] driver supports the following adapters:
...
Devices supported by the man:ufoma[4] driver include:
...
The man:uhso[4] driver should work with most devices from Option. The following devices have been verified to work
...
The man:ure[4] driver supports the following USB Ethernet controllers:
...
The man:urndis[4] driver supports the "tethering" functionality of many Android devices.
...
The man:vte[4] device driver provides support for the following Ethernet controllers:
...
The link:https://man.freebsd.org/cgi/man.cgi?query=ar40xx&amp;apropos=&amp;&sektion=4&amp;format=html[ar40xx_switch] driver supports the following Gigabit Ethernet switch controllers:
...
The man:arswitch[4] driver supports the following Ethernet switch controllers:
...
The man:e6000sw[4] driver supports the following Gigabit Ethernet switch controllers:
...
The man:e6060sw[4] driver supports the following Fast Ethernet switch controllers:
...
The man:ip17x[4] driver supports the following Fast Ethernet switch controllers:
...
The man:mtkswitch[4] driver supports the following Ethernet switch controllers:
...
The man:ath[4] driver supports all Atheros Cardbus, ExpressCard, PCI and PCIe cards, except those that are based on the AR5005VL chipset.



The following cards are among those supported by the man:ath_hal[4] module:
...
The man:bwn[4] driver supports Broadcom BCM43xx based wireless devices, including:
...
The man:ipw[4] driver provides support for the Intel PRO/Wireless 2100a/b MiniPCI network adapter.



The man:iwi[4] driver supports the following wireless network devices:
...
The man:iwlwifi[4] driver supports PCIe devices from the **mvm** sub-driver with the following chipset generations:
...
The man:iwm[4] driver supports the following PCIe Wi-Fi devices:
...
The man:iwn[4] driver provides support for:
...
The man:iwx[4] driver supports the following M.2 wireless network adapters:
...
The following cards are among those supported by the man:malo[4] driver:
...
The man:mtw[4] driver supports MediaTek MT7601U based USB wireless network adapters including (but not all of them tested):
...
The man:otus[4] driver provides support for Atheros AR9170 USB IEEE 802.11b/g/n wireless network adapters, including:
...
The man:ral[4] driver supports PCI/PCIe/CardBus wireless adapters based on Ralink Technology chipsets, including:
...
The man:rsu[4] driver provides support for Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n wireless network adapters, including:
...
The man:rtw88[4] driver supports PCIe devices with the following chipsets:
...
The man:rtw89[4] driver supports PCIe devices with the following chipsets:
...
The man:rtwn_pci[4] driver supports the following PCIe Wi-Fi devices:
...
The man:rtwn_usb[4] driver supports USB wireless network adapters based on certain Realtek RTL 8188/8192/8812 and 8821 chipsets, including:
...
The man:rum[4] driver supports USB 2.0 wireless adapters based on the Ralink RT2501USB and RT2601USB chipsets, including:
...
The man:run[4] driver supports the following wireless adapters:
...
The man:ural[4] driver supports USB 2.0 wireless adapters based on the Ralink Technology RT2500USB chipset, including:
...
The man:wpi[4] driver provides support for the Intel PRO/Wireless 3945ABG Mini PCIe network adapter.



The following devices are known to be supported by the man:zyd[4] driver:
...
The man:u3g[4] driver supports the following cellular modems:
...
The man:umb[4] driver should support any USB device implementing MBIM, including the following cellular modems:
...
The man:scc[4] driver supports the following classes of Serial Communications Controllers:
...
The man:puc[4] driver supports the following PCI/PCIe multi-port serial and parallel adapters:
...
The man:uark[4] driver supports the following adapters:
...
The man:ubsa[4] driver supports the following adapters:
...
The man:ubser[4] driver provides support for the BWCT console management serial adapters.



The man:uchcom[4] driver supports the following USB to serial UART controllers:
...
The man:ucycom[4] driver currently supports the following devices which incorporate Cypress USB to RS232 bridge chips:
...
The man:uftdi[4] driver supports the following USB to serial UART controllers:
...
The man:umcs[4] driver was tested on the following adapters:
...
The man:uplcom[4] driver supports the following devices and adapters:
...
The following devices should work with the man:uslcom[4] driver:
...
The man:uvscom[4] driver supports the following adapters:
...
The man:snd_als4000[4] driver supports the following sound cards:

*	Avance Logic ALS4000



The man:snd_atiixp[4] driver supports the following audio chipsets:
...
The man:snd_cmi[4] driver supports the following sound cards:
...
The man:snd_cs4281[4] driver supports the following sound cards:

*	Crystal Semiconductor CS4281



The man:snd_csa[4] driver supports the following sound cards:
...
The man:snd_emu10k1[4] driver supports the following sound cards:
...
The man:snd_emu10kx[4] driver supports the following sound cards:
...
The man:snd_envy24[4] driver supports the following audio devices:
...
The man:snd_envy24ht[4] driver supports the following audio devices:
...
The man:snd_es137x[4] driver supports the following sound cards:
...
The man:snd_fm801[4] driver supports audio devices based on the following chipset:

*	Forte Media FM801



The man:snd_hda[4] driver supports PCI class 04h (multimedia), subclass 03h (HDA) audio controllers and codecs compatible with the Intel High Definition Audio 1.0 specification.



The man:snd_hdspe[4] driver supports the following audio devices:
...
The man:snd_ich[4] driver supports the following audio devices:
...
The man:snd_maestro3[4] driver supports the following audio devices:
...
The man:snd_neomagic[4] driver supports the following audio devices:
...
The man:snd_solo[4] driver supports the following sound cards:
...
The man:snd_spicds[4] driver supports the following codecs:
...
The man:snd_t4dwave[4] driver supports the following audio devices:
...
The man:snd_uaudio[4] driver provides support for USB audio class devices and USB MIDI class devices.



The man:snd_via8233[4] driver supports the following audio chipsets:
...
The man:snd_via82c686[4] driver supports audio devices based on the following chipset:

*	VIA 82C686A



The man:snd_vibes[4] driver supports audio devices based on the following chipset:
...
The man:atp[4] driver provides support for the following Product IDs:
...
The man:atopcase[4] driver supports the following MacBooks produced in 2015-2018 years:
...
The man:abtn[4] driver supports extended keyboard keys (special F-keys) on the following devices:
...
Devices supported by the man:akbd[4] driver include:
...
Devices supported by the man:ams[4] driver include:
...
The man:dwcotg[4] driver supports the Synopsys DesignWare USB controller found in many embedded devices.


Драйвер man:ehci[4] поддерживает USB 2.0 контроллеры, реализующие стандарт Enhanced Host Controller Interface.


The man:ohci[4] driver supports all OHCI v1.0 compliant controllers including:
...
The following devices are supported by the man:ugold[4] driver:

*	RDing TEMPer1V1.2



The man:uhci[4] driver supports all UHCI v1.1 compliant controllers including:
...
The man:uipaq[4] driver supports the following iPAQ devices:
...
The man:umass[4] driver supports USB Mass Storage devices such as:
...
The following devices are supported by the man:urio[4] driver:
...
Devices supported by the man:umodem[4] driver include:
...
The man:rtsx[4] driver supports the following Realtek SD card readers:
...
The man:ulpt[4] driver provides support for USB printers and parallel printer conversion cables, including the following:
...
The man:uvisor[4] driver supports the following devices:
...
The man:xhci[4] driver supports `XHCI` compatible controllers having PCI class 12 (serial bus), subclass 3 (USB) and programming interface 48 (XHCI).
...
The man:fwohci[4] driver provides support for PCI/CardBus FireWire interface cards. The driver supports the following IEEE 1394 OHCI chipsets:
...
The man:sbp[4] driver supports FireWire Serial Bus Protocol 2 (SBP-2) storage devices.
...
The man:ng_ubt[4] driver supports all Bluetooth USB devices that conform with the Bluetooth specification v1.1, including:
...
The man:ccr[4] driver supports the crypto accelerator engine included on adapters based on the T6 ASIC:
...
The man:hifn[4] driver supports various cards containing the Hifn 7751, 7951, 7811, 7955, and 7956 chipsets, such as:
...
The man:qat[4] driver supports the following Intel QuickAssist Technology Engines:
...
The man:safe[4] driver supports the following SafeXcel chips:
...
The man:safexcel[4] driver supports the cryptographic acceleration functions of the Inside Secure EIP-97 device found on some Marvell systems-on-chip.
...
The man:hgame[4] driver supports HID gamepads, joysticks, and controllers such as:
...
The man:ads111x[4] driver provides support for the following devices:
...
The man:amdsbwd[4] driver supports the following chipsets:
...
The man:at45d[4] driver provides support for the following devices:
...
The man:gpioths[4] driver provides support for the following devices:
...
man:ig4[4] supports the I2C controllers based on Synopsys DesignWare IP that can be found in Intel(R) Core(TM) processors starting from the fourth generation, Intel(R) Bay Trail, Apollo Lake SoC families, and some AMD systems.



The man:intpm[4] driver supports the following chipsets:
...
The man:mx25l[4] driver supports the following spi flash memory devices:
...
The current version of the man:netfpga10g_nf10bmac[4] driver works with one PIO mode interface of the NetFPGA-10G Embedded CPU Ethernet Core version 1.00a.



The following PLX/Avago/Broadcom chips are supported by the man:ntb_hw_plx[4] driver:
...
The man:ow_temp[4] driver supports the following temperature sensors:
...
The man:superio[4] driver supports a multitude of Super I/O controllers produced by Nuvoton, formerly known as Winbond, and ITE, namely:
...
The man:u2f[4] driver supports FIDO/U2F-compatible USB security keys.



The man:udl[4] driver supports USB display devices based on the DisplayLink DL-120 and DL-160 graphic chips, including:
