...
Страницы проекта FreeBSD/ppc содержат информацию о портировании FreeBSD на архитектуру PowerPC(R). Как и сам порт, эти страницы всё ещё находятся в стадии разработки.
...
Порт FreeBSD/ppc по-прежнему является платформой link:{committers-guide}#archs[уровня 2]. Это означает, что он не получает полной поддержки нашего Директора по информационной безопасности, инженеров по подготовке релизов и сопровождающих инструментальной цепочки. Тем не менее, он поддерживается со стороны portmgr (сборка пакетов).

Самая актуальная информация о поддерживаемом аппаратном обеспечении в настоящее время ведётся https://wiki.freebsd.org/powerpc[в вики].
...
* **13 апреля 2021**: Выпущена FreeBSD 13.0, представившая новую цель `powerpc64le` и LLVM в качестве основного компилятора. Добавлена поддержка драйверов дисков, сети и видеоконтроллера для современных машин POWER от Raptor Computing (Blackbird и Talos II) и IBM (LC922 и IC922). https://www.freebsd.org/releases/13.0R/relnotes/[Информация о релизе]
* **22 сентября 2020**: Была представлена новая цель `powerpc64le` (64-разрядная с обратным порядком байт) для процессоров POWER8 и новее.
...
* **05 мая 2019**: FreeBSD была протестирована и, по всей видимости, работает на https://wiki.freebsd.org/powerpc/Raptor/Blackbird[Raptor Blackbird].
...
FreeBSD 12.x больше не поддерживается, и её использование *категорически не рекомендуется*. Изменения в ABI делают релиз 12.x бинарно несовместимым с 13.x и новее.
...
** *Пользователи 64-разрядных процессоров с двунаправленным порядком следования байтов POWER8 и POWER9*: можно выбрать между FreeBSD/powerpc64 и FreeBSD/powerpc64le (с прямым и обратным порядком следования байтов, соответственно). FreeBSD/powerpc64 (с прямым следованием байтов), как и другие 64-разрядные платформы, также поддерживает нативную работу 32-разрядных двоичных файлов.
** *Пользователи 64-разрядных процессоров с прямым следованием байтов Apple G5 и POWER7*: следует установить FreeBSD/powerpc64. Пользователи Apple G5 также могут выбрать 32-разрядную FreeBSD/powerpc, однако во FreeBSD/powerpc64 имеется поддержка ZFS и возможность использования более 2 ГБ ОЗУ.

* 32-разрядные системы
** *Пользователи 32-разрядных процессоров Apple G3/G4*: следует использовать FreeBSD/powerpc
** *Пользователи 32-разрядных процессоров Freescale MPC85XX (Routerboard RB800) и NXP QorIQ P1022 (AmigaOne A1222)*: следует использовать FreeBSD/powerpcspe

[[contact]]
=== С кем можно связаться?

Основным источником информации является <<list,список рассылки FreeBSD/ppc>>.
...
* http://www.freescale.com/files/product/doc/MPCFPE32B.pdf[Документация по 32-разрядным чипам PowerPC]
...
//
// Original EN revision (04.04.2026): 9d7e125935722c62fb2d26fa8ac3ed489808ab92
//
