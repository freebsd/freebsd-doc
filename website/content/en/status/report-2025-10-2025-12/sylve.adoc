...
package:libvirt[] for virtualization
package:qemu-tools[] for disk image management
package:samba419[] for SMB file sharing
package:swtpm[] for TPM emulation support
package:dnsmasq[] for DHCP and DNS services

==== Q4 Progress Highlights

===== Storage

* ZFS Management: we now use link:https://github.com/alchemillahq/gzfs[GitHub - gzfs] that is a go wrapper around zfs/zpool/zdb CLI to provide full support for creating and managing ZFS pools and datasets. This has improved performance and reliability compared to our previous implementation.
...
* Support for Linux Jails using the FreeBSD Linuxulator has been added. Users can create, manage, and monitor Linux Jails via the web interface. It's still considered experimental but we've tested extensively with Rocky Linux and Alpine.
...
* DHCP Server: A web UI around package:dnsmasq[] has been added, we allow creation of Ranges, Reservations, and Options.
...
We're also developing a new, tightly integrated backup system within the Sylve monorepo, enabling simple backup and restore of VMs, jails, and configurations.
