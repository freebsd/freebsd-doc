...
Instead of our current scheme where we provide 2 ports, one for the latest major version (`textproc/opensearch`, currently at version 2.8.0) and one for the legacy major version in maintenance mode(`textproc/opensearch13`, currently at version 1.3.10), we will move to distribute each minor version as a new port (i.e. `textproc/opensearch26`, `textproc/opensearch27`, `textproc/opensearch28` and so on).
