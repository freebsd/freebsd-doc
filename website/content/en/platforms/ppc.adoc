...
The easiest way to use ports on FreeBSD is to use portsnap. (if running FreeBSD {rel122-current} or {rel130-current}) or Git (if running FreeBSD-CURRENT). Refer to the Handbook if you need assistance to link:{handbook}#ports-using[use the Ports Collection].
...
** *Users of 64-bit bi-endian CPUs POWER8 and POWER9*: can choose between FreeBSD/powerpc64 and FreeBSD/powerpc64le (big endian and little endian, respectivelly). The FreeBSD/powerpc64 (big endian) also supports running 32-bit binaries nativelly like other 64-bit platforms.
...
|IBM | Power System S812L / 8247-21L (POWER8) |mailto:alfredo@freebsd.org[Alfredo Dal'Ava Júnior] | FreeBSD 13.0 | powerpc64, powerpc64le |
...
|Raptor CS | Talos II Desktop Development System (POWER9)|mailto:alfredo@freebsd.org[Alfredo Dal'Ava Júnior] | FreeBSD 13.0 | powerpc64, powerpc64le | Firmware v2.00, onboard VGA 
|Raptor CS | Talos II 2U Rack Mount Server (POWER9)|mailto:alfredo@freebsd.org[Alfredo Dal'Ava Júnior] | FreeBSD 13.0 | powerpc64, powerpc64le |Firmware v2.00
|===
...
* Raptor CS Blackbird and Talos II: recent firmware versions doesn't report onboard video framebuffer memory address on device tree anymore. Workaround is append the content bellow to the kernel arguments during boot or set it permanently in /boot/etc/kboot.conf:
:
+
....
