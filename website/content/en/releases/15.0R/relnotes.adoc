...
Add a `-f` option to `kadmin -l dump` which can be used to dump the Heimdal KDC database in a format that can be loaded into the MIT KDC.
See https://wiki.freebsd.org/Kerberos/Heimdal2MIT_KDC_Migration for how to use `-f` to transfer/convert the KDC database.
...
man:agp[4] has been planned for removal in FreeBSD 16.0, and the man page now states that it is deprecated.
