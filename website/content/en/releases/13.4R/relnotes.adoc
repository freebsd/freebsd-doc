...
This document lists significant changes to the FreeBSD kernel and userland.
...
Please read the link:../errata/[release errata] before installing or upgrading FreeBSD.
...
Clearly the release notes cannot list every single change made to FreeBSD between releases; this document focuses primarily on user-visible changes, and major architectural improvements.
