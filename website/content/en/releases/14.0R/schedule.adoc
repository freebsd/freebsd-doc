...
|Code slush begins / KBI freeze |- |- |Release Engineers announce that all further commits to the {localBranchHead} branch will not require explicit approval, however new features should be avoided. Additionally, there can be no changes to the KBI until {localBranchHead} is branched to {localBranchStable}.
|{localBranchStable} branch |- |- |{localBranchStable} branch created; release engineering continues on this branch.
|{localBranchReleng} branch |- |- |{localBranchReleng} branch created; future release engineering proceeds on this branch. Commits to this branch require explicit approval from the Release Engineers.
...
|RELEASE builds begin |- |- |{localRel}-RELEASE builds begin.
|RELEASE announcement |- |- |{localRel}-RELEASE press release.
