---
title: "FreeBSD 11.4-RELEASE Hardware Notes"
sidenav: download
---

= FreeBSD 11.4-RELEASE Hardware Notes
...
This document contains the hardware compatibility notes for FreeBSD 11.4-RELEASE. It lists the hardware platforms supported by FreeBSD, as well as the various types of hardware devices (storage controllers, network interfaces, and so on), along with known working instances of these devices.
...
If you have a system that is not listed here, it may not have been tested with FreeBSD 11.4-RELEASE. We encourage you to try it and send a note to the http://lists.FreeBSD.org/mailman/listinfo/freebsd-sparc64[FreeBSD SPARC porting mailing list] with your results, including which devices work and which do not.
...
[amd64, i386, pc98, sparc64] IDE/ATA controllers (man:ata[4] driver)

[pc98] IDE/ATA controllers (wdc driver)

* On-board IDE controller

[i386,amd64] Controllers supported by the man:aac[4] driver include:
...
[i386,amd64] Controllers supported by the man:ciss[4] driver include:
...
[i386,amd64] The man:dpt[4] driver provides support for the following RAID adapters:
...
[i386,amd64] Controllers supported by the man:iir[4] driver include:
...
[i386,amd64] The man:mfi[4] driver supports the following hardware:
...
[i386,amd64] Controllers supported by the man:mlx[4] driver include:
...
[i386,amd64] Controllers supported by the man:mly[4] driver include:
...
These controllers are supported by the man:mpr[4] driver:
...
The man:mrsas[4] driver supports the following hardware:
...
[i386,amd64] The man:tws[4] driver supports the following SATA/SAS RAID controller:
...
* Mitsumi proprietary CD-ROM interface (all models) (man:[4])
...
[i386,pc98,amd64,powerpc] Adapters supported by the man:aue[4] driver include:
...
[i386,pc98,sparc64,amd64] The man:bge[4] driver provides support for various NICs based on the Broadcom BCM570x family of Gigabit Ethernet controller chips, including the following:
...
The man:bnxt[4] driver provides support for various NICs based on the Broadcom NetXtreme-C and NetXtreme-E families of Gigabit Ethernet controller chips, including the following:
...
[i386,amd64] The man:bxe[4] driver provides support for various NICs based on the QLogic NetXtreme II family of 10Gb Ethernet controller chips, including the following:
...
[i386,pc98,amd64,powerpc] The following devices are supported by the man:cdce[4] driver:
...
[i386,pc98,amd64,powerpc] The man:cue[4] driver supports CATC USB-EL1210A based USB Ethernet adapters including:
...
The man:cxgbe[4] driver supports 100Gb and 25Gb Ethernet adapters based on the T6 ASIC:
...
The man:cxgbe[4] driver supports 40Gb, 10Gb and 1Gb Ethernet adapters based on the T5 ASIC:
...
The man:cxgbe[4] driver supports 10Gb and 1Gb Ethernet adapters based on the T4 ASIC:
...
The man:cxgbev[4] driver supports Virtual Functions on 40Gb, 10Gb and 1Gb Ethernet adapters based on the T5 ASIC:
...
The man:cxgbev[4] driver supports Virtual Functions on 10Gb and 1Gb Ethernet adapters based on the T4 ASIC:
...
[i386,pc98,amd64] Adapters supported by the man:de[4] driver include:
...
[i386] The man:e[4] driver provides supports the following 8 and 16bit ISA Ethernet cards that are based on the Intel i82586 chip:
...
The man:ixl[4] driver supports these SFP+ Pluggable Optics:

* Intel TRIPLE RATE 1G/10G/40G QSFP+ SR (bailed) E40GQSFPSR
* Intel TRIPLE RATE 1G/10G/40G QSFP+ SR (bailed) E40GQSFPLR 

The man:ixl[4] dr driver supports 10Gb and 1Gb Ethernet adapters with SR Modules:

* Intel DUAL RATE 1G/10G SFP+ SR (bailed) FTLX8571D3BCV-IT
* Intel DUAL RATE 1G/10G SFP+ SR (bailed) AFBR-703SDZ-IN2 

The man:ixl[4] dr driver supports 10Gb and 1Gb Ethernet adapters with LR Modules:
...
[i386,pc98,amd64,powerpc] The man:kue[4] driver supports Kawasaki LSI KL5KLUSB101B based USB Ethernet adapters including:
...
The man:mlx5en[4] driver supports 100Gb, 50Gb, 40Gb, 25Gb and 10Gb Ethernet adapters. ConnectX-4 supports:10/20/25/40/50/56/100Gb/s speeds. ConnectX-4 LX supports:10/25/40/50Gb/s speeds (and reduced power consumption) :
...
[i386,pc98,amd64] The man:pcn[4] driver supports adapters and embedded controllers based on the AMD PCnet/FAST, PCnet/FAST+, PCnet/FAST III, PCnet/PRO and PCnet/Home Fast Ethernet chips:
...
[amd64] The man:qlxgbe[4] driver supports 10 Gigabit Ethernet & CNA Adapter based on the following chipsets:

* QLogic 8300 series

[amd64] The man:qlxge[4] driver supports 10 Gigabit Ethernet & CNA Adapter based on the following chipsets:
...
The man:urndis[4] driver supports the functionality of many Android devices.
...
[i386,pc98,amd64] The man:vx[4] driver supports the following cards:
...
[i386, pc98] DEC DEFPA PCI (man:fpa[4] driver)
...
[i386,amd64] The man:otus[4] driver provices support for Atheros AR9170 USB IEEE 802.11b/g/n wireless network adapters, including:
...
[i386,amd64] The man:rsu[4] driver provices support for Realtek RTL8188SU/RTL8192SU USB IEEE 802.11b/g/n wireless network adapters, including:
...
Realtek RTL8188CE based PCIe IEEE 802.11b/g/n wireless network adapters (man:rtwn[4] driver)
...
The man:urtwn[4] driver supports Realtek RTL8188CU/RTL8188RU/RTL8188EU/RTL8192CU based USB IEEE 802.11b/g/n wireless network adapters, including:
...
*Note*: +
[amd64, i386, pc98] USB Ethernet adapters can be found in the section listing <<ethernet,Ethernet interfaces>>.

*Note*: +
[amd64, i386, pc98] USB Bluetooth adapters can be found in <<bluetooth,Bluetooth>> section.

[i386,pc98,amd64,powerpc] The man:ohci[4] driver supports all OHCI v1.0 compliant controllers including:
...
[i386,pc98,amd64,powerpc] The man:uhci[4] driver supports all UHCI v1.1 compliant controllers including:

* Intel 82371AB/EB (PIIX4)
* Intel 82371SB (PIIX3)
* VIA 83C572

[amd64, i386, pc98] USB 2.0 controllers using the EHCI interface (man:ehci[4] driver)

[amd64, i386, pc98] Hubs

[amd64, i386, pc98] Keyboards (man:ukbd[4] driver)
...
[amd64, i386, pc98] Mice (man:ums[4] driver)
...
[i386,sparc64,amd64,powerpc] The man:fwohci[4] driver provides support for PCI/CardBus FireWire interface cards. The driver supports the following IEEE 1394 OHCI chipsets:
...
* [amd64, i386, pc98] USB keyboards (man:ukbd[4] driver)
...
* [amd64, i386, pc98] USB mice (man:ums[4] driver)
