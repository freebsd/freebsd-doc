...
The SD card image contains the full FreeBSD installation, and can be installed onto SD cards as small as 512Mb.
+
...
Additionally, the `root` user password is set to `root`.
It is strongly recommended to change the password for both users after gaining access to the system.
+
...
Based on the new FreeBSD support model, the FreeBSD {thisBranch} release series will be supported until at least {thisEOL}.
...
Please note that {lastRelease} will be supported until three months after the {thisRelease} release date.
