...
Процесс изменения частоты дискретизации аудиопотока называется ресемплингом.

Обычно это происходит автоматически в таких ситуациях, как запись звука через DI (цифровой интерфейс) или просто при воспроизведении аудиофайлов с очень низким разрешением.

Пример: DI использует 192000 кГц, а приложение принимает только 48000 кГц (или наоборот).

Если возможно, это следует настраивать за пределами драйвера звукового устройства, но для случаев, когда это невозможно (или оборудование плохо выполняет ресемплинг), у вас есть возможность влиять на качество, используемое при ресемплинге.

Если вы не уверены, нужна ли эта настройка, то для обычного прослушивания музыки, проведения встреч и тому подобного значение по умолчанию вполне подходит, и большинство приложений в любом случае используют 48000 кГц, так что передискретизация происходит редко.

При создании музыки, записи звуков в других случаях, где качество имеет значение, или если вы аудиофил, вы можете повысить качество ресемплинга.

Для достижения максимально возможного качества передискретизации выполните следующую команду:
...
....
<.> 100 дБ в полосе задерживания, 164 отвода и 97% полосы пропускания
<.> Крупные простые деления для высокой точности

Чтобы сохранить эти значения после перезагрузки, их можно добавить в `/etc/sysctl.conf`.

Важно помнить, что изменение качества ресемплинга влияет на использование процессора и задержку.

Более подробную информацию о ресемплинге можно найти в man:sound[4]
...
| cmus — C* музыкальный проигрыватель
| GPL 2.0
| package:audio/cmus[]
| TUI
...
cmus — это небольшой, быстрый и мощный консольный музыкальный плеер для Unix-подобных операционных систем.

Помимо различных плагинов ввода и вывода, он поддерживает CUE-листы, использует Unicode, поддерживает скробблинг (например, last.fm или libre.fm), может запускать внешние программы для выбранных файлов, такие как редакторы тегов, управляется через UNIX-сокеты и многое другое.

Для установки cmus выполните:
...
mpv — это свободный (в смысле свободы, а не бесплатности) медиаплеер для командной строки. Он поддерживает широкий спектр форматов медиафайлов, аудио- и видеокодеков, а также типов субтитров. mpv может использовать большинство API аппаратного декодирования на всех платформах.

Ориентированный также на видеолюбителей, он имеет несколько высококачественных алгоритмов масштабирования вверх/вниз, которыми, как и всеми настройками, можно управлять через профили. Профили можно выбирать с помощью параметров командной строки или на основе условий, таких как имя каталога.

Она также поддерживает высококачественное преобразование HDR в SDR с тонкой настройкой управления, позволяя каждому наслаждаться HDR-контентом на любом дисплее.

Для установки mpv выполните:
...
Вот короткий пример [filename.]#~/.config/mpv/mpv.conf#
...
....
<.> Использует новый видеовыход gpu-next. Многие функции требуют этого и/или обеспечивают более высокое качество, если этот параметр включен
<.> Пользователи Wayland должны вручную установить это при появлении графических сбоев или возникновении сбоев программы
<.> Автоматически определяет доступность аппаратного декодирования GPU и использует его
<.> Наилучший алгоритм масштабирования. Отключите, если ваше оборудование недостаточно быстрое и видео начинает пропускать кадры
<.> Пример профиля для длинных видео записанных сессий TTRPG
<.> Применяется только в том случае, если путь содержит ttrpg
<.> Использует встроенный быстрый профиль для минимально возможного использования CPU/GPU
<.> Пример профиля для пользователей с медленной встроенной графикой (iGPU), которые всё таки хотят наслаждаться HDR-контентом

Используя эту примерную конфигурацию, профиль TTRPG автоматически применяется, если выполняется условие "Path contains ttrpg".

Чтобы просмотреть HDR-видео на iGPU, вручную укажите профиль с помощью следующей команды:

....
$ mpv --profile=hdr hdr_video.mkv
....

За дополнительными примерами использования mpv обращайтесь к справочному руководству mpv.
...
[NOTE]
====
Обратите внимание, что если это веб-камера с поддержкой plug-and-play USB, то изменение USB-порта, к которому она подключена, приведёт к изменению вывода команды `webcamd -l`, а именно, идентификатора устройства, и запись в rc.conf может потребовать обновления. Для ноутбуков со встроенными USB-веб-камерами это не должно быть проблемой. Чтобы избежать такой ситуации, или используйте имя устройства (опция `-N`), или/и определяйте соответствие устройства по серийному номеру, если он известен (опция `-S`) и отображается в выводе команды `webcamd -l`. Для статических устройств, например, интегрированных видеокамер ноутбуков, вы можете использовать идентификатор устройства (опция `-d`).
====
