...
<1> Секция для описания самого порта и его главного сайта: первыми указываются переменные PORTNAME и PORTVERSION или DISTVERSION*, за ними CATEGORIES, затем MASTER_SITES, после которой следует MASTER_SITE_SUBDIR. Если нужно, то после неё указываются PKGNAMEPREFIX и PKGNAMESUFFIX. Затем следуют DISTNAME, EXTRACT_SUFX и/или DISTFILES, и уже потом, если нужно, EXTRACT_ONLY.
