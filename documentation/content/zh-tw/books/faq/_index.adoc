...
這份文件是 FreeBSD 常見問答集 ( (FAQ) )。我們盡可能地讓這份 FAQ 提供有用的資訊。
...
有的。這些文件都分別以不同格式儲存以及壓縮處理並放在 FTP 上面，可以從各個 FreeBSD FTP 站的 https://download.freebsd.org/doc/[/pub/FreeBSD/doc/] 目錄內找到你要的。
