...
....
+
. Browse around in the directory structure to locate the files you wish to edit. The FreeBSD handbook files are located in `/documentation/content/en/books/handbook/`.
+
. If you feel a file requires major changes, consult the mailing list for the team's feedback before starting work.
+
. *_Always_* build and review your changes before submitting them. Run `make` in the top-level directory of the documentation (`/documentation/`) to generate documentation in HTML format.
+
[source,shell]
....
% make
....
+
To reduce build time, you can build only one language:
+
[source,shell]
....
% make DOC_LANG="en"
....
+
The build output is stored in `/documentation/public/en/books/`.
+
. Review the build output and ensure the edits are free from typos, layout problems, or errors. If you encounter errors during the build process, edit the problem files to fix any problems shown, then rerun the build command until all errors are resolved.
+
