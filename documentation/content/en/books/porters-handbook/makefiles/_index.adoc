...
In this case, use the `BUILD_RUN_DEPENDS` variable.
...
[[makefile-build_run_depends]]
=== `BUILD_RUN_DEPENDS`

This variable specifies executables or files this port requires to build and to run.
Like `RUN_DEPENDS`, it is a list of ``_path:dir_``[:``_target_``] tuples.
For example,
...
[NOTE]
====
"build_run" here means everything from configuration to compilation, plus run-time.
The dependency is checked from within the `configure` target and within the `install`.
The _target_ part can be omitted if it is the same as `DEPENDS_TARGET`
====
