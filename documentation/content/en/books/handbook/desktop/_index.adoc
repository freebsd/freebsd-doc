...
Search for them by executing the following command:
...
A keyboard language, in this case Spanish, can be set in SDDM with the following command:
...
Alternatively, start Plasma in an Xorg session is by manually invoking man:startx[1].
...
Okular combines excellent functionality with the versatility of supporting different kinds of documents, like PDF, Postscript, DjVu, CHM, XPS, ePub and others.
