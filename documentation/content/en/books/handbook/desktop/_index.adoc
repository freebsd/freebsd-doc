...
[WARNING]
====
SDDM uses a newer crossref:wayland[wayland, Wayland] display server with KDE Plasma by default.
As of August 2025, this combination does have some stability issues.
If you encounter them, choose more traditional crossref:x11[x11, X11] session from the bottom-left of the login screen.
You can also launch X11 session directly by using the startx method described below.
====
