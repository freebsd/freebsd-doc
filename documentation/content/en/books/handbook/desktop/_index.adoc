...
There is two ways to start MATE, either with a display manager or by starting it manually.

To start it with a display manager, consider using LightDM which supports different display technologies and is a good choice as it is very lightweight, requires little memory usage, and has fast performance.
...
To start MATE manually, you need to invoke man:startx[1]. For this to work, the following line is needed in `~/.xinitrc`:
