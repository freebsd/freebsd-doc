...
To start it with a display manager, consider using Light DM which supports different display technologies and is a good choice as it is very lightweight, requires little memory usage, and has fast performance.
...
To start MATE, you will need to invoke man:startx[1]. For this to work, the following line is needed in `~/.xinitrc`:
