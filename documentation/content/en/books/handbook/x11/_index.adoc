...
This chapter describes how to install and configure X.Org, which provides the open source X Window System used to provide a graphical environment.
It then describes how to find and install a desktop environment or window manager.

[NOTE]
====
Users who prefer an installation method that automatically configures X.Org may refer to https://ghostbsd.org[GhostBSD], https://www.midnightbsd.org[MidnightBSD] or https://nomadbsd.org[NomadBSD].
====

For more information on the video hardware that X.Org supports, refer to the https://www.x.org/wiki/[X.Org] website.

After reading this chapter, you will know:

* The various components of the X Window System, and how they interoperate.
* How to install and configure X.Org.
* How to install and configure several window managers and desktop environments.
* How to use TrueType(R) fonts in X.Org.
...
On FreeBSD, X.Org can be installed as a package or port.
...
Alternatively, to build and install from the Ports Collection:
...
Either of these installations results in the complete X.Org system being installed.
Binary packages are the best option for most users.

A smaller version system suitable for experienced users is available in package:x11/xorg-minimal[].
...
X.Org supports most common video cards, keyboards, and pointing devices.
...
[.procedure]
. If X.Org has been used on this computer before, move or remove any existing configuration files:
+
[source,shell]
....
...
. Add the user who will run X.Org to the `video` or `wheel` group to enable 3D acceleration when available. To add user _jru_ to whichever group is available:
+
[source,shell]
....
...
. The `TWM` window manager is included by default. It is started when X.Org starts:
+
[source,shell]
....
% startx
...
For those, fill in the `-xxx` using the https://download.nvidia.com/XFree86/FreeBSD-x86_64/465.19.01/README/[ Supported NVIDIA GPU Products ] page.
...
Read https://download.nvidia.com/XFree86/FreeBSD-x86_64/465.19.01/README/[installation and configuration of NVIDIA driver] for details.
...
X.Org attempts to use this driver when a specific driver is not found for the video card.
+
...
X.Org uses `EDID` to communicate with the monitor and detect the supported resolutions and refresh rates.
...
In some cases, X.Org autoconfiguration does not work with particular hardware, or a different configuration is desired.
...
[[x-fonts]]
== Using Fonts in X.Org

[[type1]]
=== Type1 Fonts

The default fonts that ship with X.Org are less than ideal for typical desktop publishing applications.
Large presentation fonts show up jagged and unprofessional looking, and small fonts are almost completely unintelligible.
However, there are several free, high quality Type1 (PostScript(R)) fonts available which can be readily used with X.Org.
...
In addition, X.Org can be configured to use TrueType(R) fonts with a minimum of effort.
...
Alternatively, to build from the Ports Collection, run the following command:
...
X.Org includes support for rendering TrueType(R) fonts.
...
All fonts in X.Org that are found in [.filename]#/usr/local/share/fonts/# and [.filename]#~/.fonts/# are automatically made available for anti-aliasing to Xft-aware applications. Most recent applications are Xft-aware, including KDE, GNOME, and Firefox.
...
X.Org provides an X Display Manager, XDM, which can be used for login session management.
...
It can be installed from a package:
...
To instead build from ports, use the following command.
GNOME is large and will take some time to compile, even on a fast computer.
...
GNOME uses the https://www.freedesktop.org/wiki/Software/dbus/[D-Bus] message bus system. 
D-Bus is automatically installed as a dependency of GNOME.
Edit [.filename]#/etc/rc.conf# for D-Bus to start when the system boots:
...
After installation, configure X.Org to start GNOME.
...
Edit [.filename]#/etc/rc.conf# for GDM to start when the system boots:
...
Create (or overwrite) [.filename]#~/.xsession#:
...
https://kde.org/plasma-desktop/[KDE Plasma] is another easy-to-use desktop environment.
It provides a suite of applications with a consistent look and feel, a standardized menu and toolbars, keybindings, color schemes, internationalization, a centralized, dialog-driven desktop configuration, and integral help.
For more information on KDE, see https://www.kde.org/[https://www.kde.org/] and FreeBSD-specific https://freebsd.kde.org/[https://freebsd.kde.org].
...
Alternatively, to build KDE, use the following command.
You will be prompted to choose components.
Building the required collection of ports will be time-consuming. 
...
KDE uses D-Bus. 
D-Bus is automatically installed as a dependency.
Edit [.filename]#/etc/rc.conf# for D-Bus to start when the system boots:
...
To install SDDM, the display manager that is most commonly used to start Plasma:
...
When SDDM appears: before you login, to avoid possible problems with Wayland, use the session menu to switch from _Plasma&#160;(Wayland)_ to _Plasma&#160;(X11)_. 

If you prefer XDM instead of SDDM, create (or overwrite) [.filename]#~/.xsession# as follows:
...
If you prefer to not use a display manager, you can start Plasma with `startx` at the command line.
Edit [.filename]#~/.xinitrc# to include this line:
...
It is more lightweight and provides a simple, efficient, easy-to-use desktop.
...
More information on Xfce can be found at https://www.xfce.org/[https://www.xfce.org].
...
Unlike GNOME, Xfce does not provide a login manager.
To start Xfce from the command line by typing `startx`, first create (or overwrite) [.filename]#~/.xinitrc# with this command:
...
However, it has been noted that some versions of X.Org will not function properly if the driver is not loaded at boot time.
...
In modern versions of X.Org, the `InputDevice` sections in [.filename]#xorg.conf# are ignored in favor of the autodetected devices.
...
[NOTE]
====
One of the tools available to assist you during troubleshooting process are the X.Org log files, which contain information on each device that the X.Org server attaches to.
X.Org log file names are in the format of [.filename]#/var/log/Xorg.0.log#.
...
The X.Org configuration process is now complete.
X.Org may be now started with the man:startx[1] utility.
The X.Org server may also be started with the use of man:xdm[8].

=== Configuration with Intel(R) `i810` Graphics Chipsets

Configuration with Intel(R) i810 integrated chipsets requires the [.filename]#agpgart# AGP programming interface for X.Org to drive the card.
...
X.Org is smart enough to pull the resolution information from the widescreen via I2C/DDC information so it knows what the monitor can handle as far as frequencies and resolutions.

If those `ModeLines` do not exist in the drivers, one might need to give X.Org a little hint.
...
This is usually the case when you upgrade X.Org.
You will need to reinstall the package:x11/nvidia-driver[] package so glx is built again.

[[x-wayland]]
== Wayland on FreeBSD
Wayland is a new software for supporting graphical user interfaces, but it differs from X.Org in several important ways.
...
X.Org includes both the X11 protocol, used to run remote displays and the X server will accept connections and display windows.
...
Earlier in this document it was noted that Wayland does not provide the same X server style access as X.Org provides.
