...
* Use man:portsnap[8] for most use cases. Refer to link:{handbook}#ports-using/[Using the Ports Collection] for instructions on how to use this tool.
* Use Git if custom patches to the ports tree are needed or if running FreeBSD-CURRENT. Refer to link:{handbook}mirrors/#git[Using Git] for details.
