...
. `nano_fetch_pkgbase_packages` or `nano_fetch_distsets`: Obtain the base system, either as FreeBSD-base (pkgbase) packages, or as distribution sets such as [.filename]#base.txz# and [.filename]#kernel.txz# fetched from a FreeBSD mirror and verified against `MANIFEST`. For an unprivileged build (`-U`), the matching metalog is generated at this point by `nano_pkgbase_reset_metalog` or `nano_distset_metalog`.
. `clean_world`: Clean the destination directory.
. `make_conf_install`: Assemble make.conf from the `CONF_WORLD` and `CONF_INSTALL` variables. (Only for a source build.)
...
. `fixup_before_diskimage`: Assemble and deduplicate the metalog of all installed files, normalizing ownership and stripping the `size` and `time` keywords so later customizations do not invalidate it.
. `calculate_partitioning`: Compute the partition/slice layout (code, configuration, data, and, for the GPT plan, boot and EFI System Partitions) from the configured sizes.
. `create_code_partition`: Lay out and populate the code partition (or slice) that holds the image.
. `create_diskimage` or `_create_diskimage`: Creates the actual disk image, based on the disk geometry parameters. The `_create_diskimage` variant is used for unprivileged builds (`-U`), where the image is assembled from the metalog with man:makefs[8] instead of real device nodes.
...
* `NANO_PLAN` - Selects the partitioning and boot scheme. When left unset, the legacy man:gpart[8] MBR layout is used (a warning is printed). Calling the `gpt` function in the configuration file switches to a GPT layout. The legacy plan still drives the older variables `NANO_NEWFS`, `NANO_BOOT0CFG`, and `NANO_BOOTLOADER`, while the GPT plan uses `NANO_MAKEFS` and adds the variables below.
* `NANO_BOOT_TYPE` - For the GPT plan, a space-separated list of boot methods to support: `BIOS`, `UEFI`, or both (the default).
...
* `NANO_NOPRIV_BUILD` - Set automatically by `-U`; builds the image without `root` by recording metadata in a metalog.
...
* `NANO_TIMESTAMP` - Timestamp applied to files, by default the time of the last commit in the source tree.

Other frequently useful options:

* `NANO_MAKEFS` - Parameters passed to man:makefs[8] when building the image for the default/GPT plan (default `-o softupdates=1,version=2`). This replaces the older `NANO_NEWFS`, which is now used only by the legacy plan.
* `NANO_SECTOR_SIZE` - Logical sector size, in bytes, of the target media.
* `NANO_SWAP_SIZE` - Size of an optional swap partition, in 512-byte sectors (`0` disables it).
