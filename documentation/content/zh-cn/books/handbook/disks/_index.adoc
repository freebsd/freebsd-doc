...
接下来考虑的是，如何创建文件系统。现代 FreeBSD 推荐使用 man:geom[4] 或 ZFS。
...
// Vinum 卷管理相关内容已从手册中移除。现代 FreeBSD 推荐使用 ZFS（推荐）或 GEOM 框架进行存储管理。请参考 zfs 和 geom 章节。
...
还需要注意的是，旧的 vinum 不使用 geom 子系统。现代方案请使用 ZFS 加密或 geom 相关功能。
